                                ...
                                for (col_idx, val) in current_row.iter().enumerate() {
                                    if col_idx >= 2 && val == "X" {
                                        ...
                                        if visit_idx < visit_codes.len() {
                                            ...
                                            if let Some(visit) = context
                                                .visits
                                                .iter_mut()
                                                .find(|v| v.code == *visit_code)
                                            {
...
pub fn find_master_matrix_sheet<R: std::io::BufRead>(
    reader: &mut Reader<R>,
) -> Result<String, AlsParseError> {
    let mut buffer = Vec::new();
    let mut current_row: Vec<String> = Vec::new();
    let mut in_data_cell = false;
    let mut current_cell_index = 0;
            ...
            Ok(Event::End(e)) => {
                match e.name().as_ref() {
                    b"Row" => {
                        ...
                        if current_row.len() >= 2 && row_count > 1 {
                            let oid = current_row[1].clone();
                            if oid.to_uppercase().eq("MASTER") {
                                return Ok(format!("Matrix{}#MASTER", row_count - 1));
                            }
                        }
                        current_row.clear();
                    }
    ...
    Err(AlsParseError::SheetNotFound("Matrix#MASTER".into()))
}
